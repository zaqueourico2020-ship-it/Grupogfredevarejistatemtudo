...
function makeGeoNotes(geo: { estado: string; cidade: string; regiao: string; bairro: string; notes?: string }) {
  return `_GEO_:${JSON.stringify({
    estado: geo.estado,
    cidade: geo.cidade,
    regiao: geo.regiao,
    bairro: geo.bairro,
...
function PartnerProducts() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const geoNotesValue = makeGeoNotes({
        ...
        <div className="fixed inset-0 z-50 block h-[100dvh] touch-pan-y overflow-y-scroll overscroll-contain bg-black/50 px-0 py-0 [-webkit-overflow-scrolling:touch] sm:px-4 sm:py-6">
          <div className="mx-auto flex min-h-[100dvh] w-full flex-col bg-card text-card-foreground sm:min-h-[calc(100dvh-3rem)] sm:max-w-3xl sm:rounded-xl sm:border sm:border-border">
            ...
            <div className="flex-1 p-4 pb-28 space-y-4">
              ...
              <div className="bg-slate-50 dark:bg-slate-900/30 p-3 rounded-xl border border-border/60">
                <p className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider">Localização do Serviço / Atendimento (Filtros Regionais)</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <Field label="Cidade">
                    <input className="input" placeholder="Ex: São Paulo" value={form.cidade} onChange={(e) => setForm({ ...form, cidade: e.target.value })} />
                  </Field>
                  <Field label="Região">
                    <input className="input" placeholder="Ex: Zona Sul" value={form.regiao} onChange={(e) => setForm({ ...form, regiao: e.target.value })} />
                  </Field>
                  <Field label="Bairro">
                    <input className="input" placeholder="Ex: Pinheiros" value={form.bairro} onChange={(e) => setForm({ ...form, bairro: e.target.value })} />
                  </Field>
                </div>
                <p className="text-[10px] text-muted-foreground mt-2">Os usuários poderão encontrar seus serviços filtrando por estes locais específicos.</p>
              </div>
