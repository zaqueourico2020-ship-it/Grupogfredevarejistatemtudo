...
function StorePage() {
  ...
  const parseGeoNotes = (notesStr?: string) => {
                  ...
                  <div className="p-2">
                    ...
                    {p.subcategory && (
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setCategory(p.category || "Todos");
                          setSubcategory(p.subcategory ?? null);
                          setSection("home");
                        }}
                        className="mt-0.5 text-[10px] text-slate-400 hover:text-cyan-400 transition-colors cursor-pointer text-left block"
                      >
                        {p.subcategory}
                      </button>
                    )}
      ...
      {sidebarOpen && (
        <div className="absolute inset-0 z-50 flex">
          ...
          <aside className="relative h-full w-[86%] max-w-[360px] overflow-y-auto bg-[#0a1628] ring-1 ring-cyan-500/20">
            ...
            <Link 
              to="/carteira"
              className="mx-4 mb-4 rounded-xl bg-[#0f1d32] p-3 ring-1 ring-cyan-500/15 cursor-pointer hover:bg-slate-800 transition-colors block text-left"
            >
              <div className="mb-2 flex items-center justify-between">
              ...
              <div className="grid grid-cols-3 gap-2 text-[11px]">
                <div>
                  <p className="text-slate-300">Saldo disponível</p>
                  <p className="font-bold text-emerald-400">R$ 0,00</p>
                </div>
                <div>
                  <p className="text-slate-300">Saldo pendente</p>
                  <p className="font-bold text-orange-400">R$ 0,00</p>
                </div>
                <div>
                  <p className="text-slate-300">Meu cashback</p>
              ...
              <div className="mt-1 flex items-center gap-2 text-xs text-slate-400">
                {activeProduct.category && (
                  <button
                    ...
                    className="rounded bg-[#0f1d32] px-2 py-0.5 ring-1 ring-cyan-500/15 hover:bg-slate-800 text-cyan-400 transition-colors cursor-pointer font-medium"
                  >
                    {activeProduct.category}
                  </button>
                )}
                {activeProduct.subcategory && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setSubcategory(activeProduct.subcategory);
                      setSection("home");
                      setActiveProduct(null);
                    }}
                    className="rounded bg-[#0f1d32] px-2 py-0.5 ring-1 ring-cyan-500/15 hover:bg-slate-800 text-cyan-400 transition-colors cursor-pointer font-medium"
