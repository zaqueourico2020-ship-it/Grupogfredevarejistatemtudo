  ...
  .handler(async ({ data }) => {
    ...
    const [{ data: prods }, { data: orders }] = await Promise.all([
      admin.from("partner_products")
        .select("id, name, description, price, discount_price, brand, images, image_url, stock_quantity, category, subcategory, active, notes, product_variants(id,name,sku,price,discount_price,stock,image_url,attributes)")
